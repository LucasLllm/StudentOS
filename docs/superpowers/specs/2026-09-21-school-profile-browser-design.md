...
1. Reads the page's origin and finds the connected site it belongs to: the site's own host or a subdomain of it, over https. Measured on Studyo: the site is `studyo.app` and its sign-in form is on `accounts.studyo.app`, so an exact match refused to fill it; a site's subdomains are the site, the rule a browser's password manager uses too. Then it reads that site's username and password from the keychain. No match: it fails with a reason the agent can pass on, that there is no saved sign-in for that host, that the student can sign in once in the browser card and it stays signed in, or save one under Settings, Connections, Sites.
...
The two guarantees behind the old refusal stay true. The agent never receives a password: the keychain is read on the student's computer, at the moment of typing, and the value goes into the page and nowhere else. A saved password can only go to the site it was saved for: its host or a subdomain of it, over https, never a host that merely contains the name and never the site's parent, so no page can talk the agent into typing it somewhere else.

A site that signs in through Google is the ordinary case, not the exception, so the machine carries the saved sign-in through it. When a connected site's own sign-in hands off to `accounts.google.com`, `sign_in` there uses that site's saved sign-in too: the login the student saved for the site is the Google login the site asked for. This holds only while that site's own sign-in is under way, tracked as the flow the current conversation's browsing is in; a bare visit to Google, or a redirect to any other site, gets nothing. This is a deliberate widening of the origin rule, made because the student asked for it directly: they saved the login and want the agent to use it rather than sign in by hand.

Where the Google account has two-step verification, the student still finishes it: a typed password cannot answer a prompt on their phone. The machine types the email and password and lands on the two-step page; the student taps their phone, in the card, once, and the profile holds the session afterwards. So the by-hand step, where it happens at all, is the second factor, not the password.
